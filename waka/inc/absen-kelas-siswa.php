...
$ids = abs((int) $ids);
$kelas = select("DISTINCT nama_siswa, kelas, ids", "tbl_rekap_siswa", "ids = '$ids'");
$no = 1;
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $(".row > .col-sm-6:first").append('   <a href="export.php?data=absensi-kelas-siswa&id=<?= $ids; ?>" class="btn btn-default">Export Ms. Excel</a>');
  });
</script>
<div class="col-md-8">
  <table class="table table-bordered" id="list-data" border="1">
    ...
    <?php
      while ($x = mysqli_fetch_object($kelas)):
        ...
        $c_hdr = hitung_absen("kehadiran", "tbl_rekap_siswa", "kelas='$x->kelas' AND ids = '$x->ids' AND kehadiran = 1");
        $thdr = mysqli_fetch_object($c_hdr);

        $c_izn = hitung_absen("kehadiran", "tbl_rekap_siswa", "kelas='$x->kelas' AND ids = '$x->ids' AND kehadiran = 2");
        $tizn = mysqli_fetch_object($c_izn);

        $c_tgs = hitung_absen("kehadiran", "tbl_rekap_siswa", "kelas='$x->kelas' AND ids = '$x->ids' AND kehadiran = 3");
        $ttgs = mysqli_fetch_object($c_tgs);

        $c_skt = hitung_absen("kehadiran", "tbl_rekap_siswa", "kelas='$x->kelas' AND ids = '$x->ids' AND kehadiran = 4");
        $tskt = mysqli_fetch_object($c_skt);

        $c_ln = hitung_absen("kehadiran", "tbl_rekap_siswa", "kelas='$x->kelas' AND ids = '$x->ids' AND kehadiran = 5");
